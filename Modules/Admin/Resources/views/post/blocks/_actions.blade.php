...
@if ($post->isPublished() && !$post->is_trending_now)
    <form id="post-top-{{ $post->id }}" action="{{ route('admin.post.update.main', $post->id) }}" method="POST"
          class="d-inline">
        @csrf
        @method('put')
    </form>
@endif
<form id="post-destroy-{{ $post->id }}" action="{{ route('admin.post.destroy', $post->id) }}" method="post">
    @csrf
    @method('delete')
</form>
